classdef Node
   properties
      ...
      y;
      y_hat;
      c = 0;%1/36;  % 1/36 is Absorption coefficient of water 
      Ad = 60;  %scaling constant
   ...
   methods
      function obj = Node(pos,orient,scan_alternation_flag)
         if nargin > 0
            obj.position = pos;
            ...
            obj = obj.update_scan_parameters(scan_alternation_flag, 8, 60);
         end 
         
          %ekf parameters
          obj.P_prior = 0.01*[10,0,0;0,50,0;0,0,50];
          %obj.Q = 100*[0.1,0,0;0,1,0;0,0,1;];
          obj.P = obj.P_prior;
          obj.x_hat_prior = [0.9,0,0]';
          ...
          obj.R = obj.R_system;
          obj.Q_system = [0.00001,0,0;0,0.0005,0;0,0,0.0005;];
          obj.Q = obj.Q_system;;
          obj.difference = 0;
          obj.x = [0;0;0];
          obj.y = 0;
      ...
      function obj = runExtremumSeeking(obj,other_bot)
          obj = obj.update_relative_parameters(other_bot);
          global ind;
          global T;
          obj = obj.calculate_states();
          ...
          global num_iteration;
            if(nargin>1)    %nargin gives total number of inputs not just the length of varargin
                ...
                if(counter == k_factor+1)
                    ...
                    if (ind>num_iteration/2000)
                      min_ind = max(ind-k_factor + 1,1);
                      p_ave = sum(obj.history.P_eig(min_ind:ind))/k_factor;
                      obj.scan.r_adaptive = max(2,min(obj.scan.max_r,floor((p_ave)/200)));
                    end
                end
